...
class Rubik:
    ...
    def move2layers(self, direction, face):
        self.rotate(direction, face)
        self.move(direction, OPPOSITE[face])

    def transform(self, direction, face, action):
        if action == ROTATE:
            self.rotate(direction, face)
        elif action == MOVE:
            self.move(direction, face)
        elif action == MOVE2LAYERS:
            self.move2layers(direction, face)
